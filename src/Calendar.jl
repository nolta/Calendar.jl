...
function ymd_hms(s::AbstractString, tz=_tz)
    ...
    y  = round(Int, m.captures[1])
    mo = round(Int, m.captures[2])
    d  = round(Int, m.captures[3])
    h  = round(Int, m.captures[4])
    mi = round(Int, m.captures[5])
    s  = round(Int, m.captures[6])
...
function colon(t1::CalendarTime, d::FixedCalendarDuration, t2::CalendarTime)
    n = floor(Int, (t2.millis - t1.millis)/d.millis) + 1
...
function colon(t1::CalendarTime, d::CalendarDuration, t2::CalendarTime)
    ...
    n = floor(Int, (t2.millis - t1.millis)/approx_d) + 1
    CalendarTimeRange(t1, d, n)
end
